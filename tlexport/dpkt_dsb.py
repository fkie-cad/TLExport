...
class DecryptionSecretBlock(dpng._PcapngBlock):
    ...
    def __bytes__(self):
        ...
        hdr_buf = self._pack_hdr(
            self.type, n, self.secrets_type, self.secrets_length, n
        )
...
class Reader(object):
    ...
    def __init__(self, fileobj):
        ...
        if shb.v_major != dpng.PCAPNG_VERSION_MAJOR:
            raise ValueError(
                "unknown pcapng version {0}.{1}".format(
                    shb.v_major,
                    shb.v_minor,
                )
            )
        ...
        while 1:
            ...
            if blk_type == dpng.PCAPNG_BT_IDB:
                idb = (
                    dpng.InterfaceDescriptionBlockLE(buf)
                    if self.__le
        ...
        for opt in idb.opts:
            if opt.code == dpng.PCAPNG_OPT_IF_TSRESOL:
                ...
                self._tsoffset = dpng.struct_unpack(
                    "<q" if self.__le else ">q", opt.data
                )[0]
    ...
    def __iter__(self):
        ...
        while 1:
            ...
            if blk_type == dpng.PCAPNG_BT_EPB:
                epb = (
                    dpng.EnhancedPacketBlockLE(buf)
                    if self.__le
                    else dpng.EnhancedPacketBlock(buf)
                )
                ts = self._tsoffset + (
                    ((epb.ts_high << 32) | epb.ts_low) / self._divisor
                )
                ...
                dsb = (
                    DecryptionSecretBlockLE(buf)
                    if self.__le
                    else DecryptionSecretBlock(buf)
                )
                ts = -1
