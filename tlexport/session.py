...
class Session:
    ...
    def find_session_secrets(self):
        ...
        if is_handshake_secret < 2 and self.tls_version == TlsVersion.TLS13:
            ...
            logging.warning(
                f"Missing CLIENT_HANDSHAKE_TRAFFIC_SECRET and SERVER_HANDSHAKE_TRAFFIC_SECRET"
            )
    ...
    def generate_keys(
        self, tls_version, server_cipher_suite, client_random, server_random
    ):
        ...
        cipher_suite = cipher_suite_parser.split_cipher_suite(
            bytes(server_cipher_suite)
        )
        ...
        match tls_version:
            case TlsVersion.TLS13:
                keys = key_derivator.dev_tls_13_keys(
                    secret_list, key_length, cipher_suite["MAC"]()
                )

            case TlsVersion.TLS12:
                if secret.label == "CLIENT_RANDOM":
                    keys = key_derivator.dev_tls_12_keys(
                        bytes.fromhex(secret.value),
                        client_random,
                        server_random,
                        key_length,
                        mac_length,
                        2 * key_length + 2 * mac_length,
                        cipher_suite["CryptoAlgo"][0],
                        cipher_suite["Mode"][1],
                        cipher_suite["MAC"],
                    )
                elif secret.label == "RSA":
                    master_secret = key_derivator.gen_master_secret_tls_12(
                        bytes.fromhex(secret.value), client_random, server_random
                    )
                    keys = key_derivator.dev_tls_12_keys(
                        master_secret,
                        client_random,
            ...
            case TlsVersion.TLS10 | TlsVersion.TLS11:
                if secret.label == "CLIENT_RANDOM":
                    keys = key_derivator.dev_tls_10_11_keys(
                        bytes.fromhex(secret.value),
                        server_random,
                        client_random,
                        key_length,
                    ...
                    master_secret = key_derivator.gen_master_secret_tls_10_11(
                        bytes.fromhex(secret.value), client_random, server_random
                    )
                    keys = key_derivator.dev_tls_10_11_keys(
                        master_secret,
            ...
            case TlsVersion.SSL30:
                if secret.label == "CLIENT_RANDOM":
                    keys = key_derivator.dev_ssl_30_keys(
                        bytes.fromhex(secret.value),
                        server_random,
                        client_random,
                        key_length,
                        ...
                        2 * key_length + 2 * mac_length,
                        cipher_suite["CryptoAlgo"][0],
                        cipher_suite["CryptoAlgo"][1],
                    )
                elif secret.label == "RSA":
                    master_secret = key_derivator.gen_master_secret_ssl_30(
                        bytes.fromhex(secret.value), client_random, server_random
                    )
                    keys = key_derivator.dev_ssl_30_keys(
                        master_secret,
        ...
        self.decryptor = Decryptor(
            ...
            keys,
            self.tls_version,
            cipher_suite["KeyLength"],
            cipher_suite["MAC"].digest_size,
            cipher_suite["TagLength"],
            block_size,
            self.extensions,
    ...
    def matches_session(self, packet: Packet):
        ...
        """
        if (
            packet.ip_src == self.server_ip
            and packet.sport == self.server_port
            and packet.ip_dst == self.client_ip
            and packet.dport == self.client_port
        ):
            return True
        elif (
            packet.ip_src == self.client_ip
            and packet.sport == self.client_port
            and packet.ip_dst == self.server_ip
            and packet.dport == self.server_port
        ):
            return True
        return False
    ...
    def decrypt(self):
        print(
            f"[*] Decrypting session: [{self.binary_to_ip(self.server_ip)}:{self.server_port}-{self.binary_to_ip(self.client_ip)}:{self.client_port}]\n"
        )
    ...
    def handle_tls_server_hello(self, record: TlsRecord):
        ...
        while extensions_index < extensions_length:
            extension_length = int.from_bytes(
                extensions_bin[extensions_index + 2 : extensions_index + 4], "big"
            )
            self.extensions[
                bytes(extensions_bin[extensions_index : extensions_index + 2])
            ] = extensions_bin[
                extensions_index + 4 : extensions_index + 4 + extension_length
            ]
        ...
        match int.from_bytes(record.record_version, "big"):
            ...
            case _:
                if int.from_bytes(record.binary[4:6], "big") == 0x0301:
                    ...
                    self.can_decrypt = False
        self.generate_keys(
            self.tls_version, self.ciphersuite, self.client_random, self.server_random
        )
    ...
    def handle_tls_13_application_record(self, record: TlsRecord, isserver):
        try:
            ...
            logging.warning(
                f"Could not decrypt Record: {self.tls_version} Application Record"
            )
    ...
    def extract_server_buf(self):
        ...
        for i in range(0, len(self.server_packet_buffer) - 1):
            if (
                self.server_packet_buffer[i].seq
                + len(self.server_packet_buffer[i].tls_data)
    ...
    def extract_client_buf(self):
        ...
        for i in range(0, len(self.client_packet_buffer) - 1):
            if (
                self.client_packet_buffer[i].seq
                + len(self.client_packet_buffer[i].tls_data)
