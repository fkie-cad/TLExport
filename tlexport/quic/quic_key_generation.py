...
def dev_quic_keys(
    key_length, secret_list, hash_fun: hashes.HashAlgorithm, quic_version: QuicVersion
):
    ...
    for secret in secret_list:
        if secret.label == "CLIENT_HANDSHAKE_TRAFFIC_SECRET":
            client_handshake_key = HKDFExpand(hash_fun, key_length, key_info).derive(
                bytes.fromhex(secret.value)
            )
            client_handshake_iv = HKDFExpand(hash_fun, 12, iv_info).derive(
                bytes.fromhex(secret.value)
            )
            client_handshake_hp = HKDFExpand(hash_fun, key_length, hp_info).derive(
                bytes.fromhex(secret.value)
            )

        elif secret.label == "SERVER_HANDSHAKE_TRAFFIC_SECRET":
            server_handshake_key = HKDFExpand(hash_fun, key_length, key_info).derive(
                bytes.fromhex(secret.value)
            )
            server_handshake_iv = HKDFExpand(hash_fun, 12, iv_info).derive(
                bytes.fromhex(secret.value)
            )
            server_handshake_hp = HKDFExpand(hash_fun, key_length, hp_info).derive(
                bytes.fromhex(secret.value)
            )

        elif secret.label == "CLIENT_TRAFFIC_SECRET_0":
            client_application_key = HKDFExpand(hash_fun, key_length, key_info).derive(
                bytes.fromhex(secret.value)
            )
            client_application_iv = HKDFExpand(hash_fun, 12, iv_info).derive(
                bytes.fromhex(secret.value)
            )
            client_application_hp = HKDFExpand(hash_fun, key_length, hp_info).derive(
                bytes.fromhex(secret.value)
            )
            client_application_secret = bytes.fromhex(secret.value)

        elif secret.label == "SERVER_TRAFFIC_SECRET_0":
            server_application_key = HKDFExpand(hash_fun, key_length, key_info).derive(
                bytes.fromhex(secret.value)
            )
            server_application_iv = HKDFExpand(hash_fun, 12, iv_info).derive(
                bytes.fromhex(secret.value)
            )
            server_application_hp = HKDFExpand(hash_fun, key_length, hp_info).derive(
                bytes.fromhex(secret.value)
            )
            server_application_secret = bytes.fromhex(secret.value)

        elif secret.label == "CLIENT_EARLY_TRAFFIC_SECRET":
            client_early_traffic_key = HKDFExpand(
                hash_fun, key_length, key_info
            ).derive(bytes.fromhex(secret.value))
            client_early_traffic_iv = HKDFExpand(hash_fun, 12, iv_info).derive(
                bytes.fromhex(secret.value)
            )
            client_early_traffic_hp = HKDFExpand(hash_fun, key_length, hp_info).derive(
                bytes.fromhex(secret.value)
            )

        elif secret.label == "SERVER_EARLY_TRAFFIC_SECRET":
            server_early_traffic_key = HKDFExpand(
                hash_fun, key_length, key_info
            ).derive(bytes.fromhex(secret.value))
            server_early_traffic_iv = HKDFExpand(hash_fun, 12, iv_info).derive(
                bytes.fromhex(secret.value)
            )
            server_early_traffic_hp = HKDFExpand(hash_fun, key_length, hp_info).derive(
                bytes.fromhex(secret.value)
            )
...
def dev_initial_keys(connection_id: bytes, quic_version: QuicVersion, chacha20: bool):
    ...
    initial_secret = HKDF(hash_fun, salt=initial_salt, length=32, info=None)._extract(
        connection_id
    )

    client_initial = HKDFExpand(hash_fun, 32, info=make_info(b"client in", 32)).derive(
        initial_secret
    )
    server_initial = HKDFExpand(hash_fun, 32, info=make_info(b"server in", 32)).derive(
        initial_secret
    )
    ...
    initial_keys = {
        "client_initial_key": HKDFExpand(
            hash_fun, key_length, make_info(key_label, key_length)
        ).derive(client_initial),
        "client_initial_iv": HKDFExpand(hash_fun, 12, make_info(iv_label, 12)).derive(
            client_initial
        ),
        "client_initial_hp": HKDFExpand(
            hash_fun, key_length, make_info(hp_label, hp_key_length)
        ).derive(client_initial),
        "server_initial_key": HKDFExpand(
            hash_fun, key_length, make_info(key_label, key_length)
        ).derive(server_initial),
        "server_initial_iv": HKDFExpand(hash_fun, 12, make_info(iv_label, 12)).derive(
            server_initial
        ),
        "server_initial_hp": HKDFExpand(
            hash_fun, key_length, make_info(hp_label, hp_key_length)
        ).derive(server_initial),
    }
...
def key_update(
    ...
    hash_fun = hash_fun()
    server_n_1 = HKDFExpand(hash_fun, hash_fun.digest_size, ku_info).derive(server_n)
    client_n_1 = HKDFExpand(hash_fun, hash_fun.digest_size, ku_info).derive(client_n)
