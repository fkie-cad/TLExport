...
def remove_header_protection(
    ...
    if header_type == QuicHeaderType.LONG:
        first_packet_byte = byte_xor(
            bytes([first_packet_byte]), byte_and(bytes([mask[0]]), bytes.fromhex("0f"))
        )
    else:
        first_packet_byte = byte_xor(
            bytes([first_packet_byte]), byte_and(bytes([mask[0]]), bytes.fromhex("1f"))
        )

    pn_len = (
        decode_variable_length_int(
            byte_and(bytes([int.from_bytes(first_packet_byte, "big")]), b"\x03")
        )
        + 1
    )
    packet_number_field = byte_xor(
        datagram_data[pn_offset : pn_offset + pn_len], mask[1 : pn_len + 1]
    )

    return first_packet_byte, packet_number_field, pn_len


def extract_quic_packet(
    in_packet: Packet,
    isserver,
    guessed_dcid: bytes = None,
    keys: {} = None,
    ciphersuite: bytes = None,
):
    packet_buf = []
    ...
    try:
        ...
        if (
            int.from_bytes(datagram_data, "big") == 0
        ):  # If we have a zero-padding at the end
        ...
        match header_type:
            case QuicHeaderType.LONG:  # if header is long header
                ...
                match packet_type:  # checks if packet type
                    case QuicPacketType.VERSION_NEG:
                        header_parts = struct.unpack_from(
                            fmt_string + "ssss", datagram_data
                        )
                        packet = LongQuicPacket(
                            packet_type=QuicPacketType.VERSION_NEG,
                            version=version,
                            dcid_len=dcid_len,
                            dcid=dcid,
                            scid_len=scid_len,
                            scid=scid,
                            supported_version=header_parts[-1:-5],
                            first_byte=header_parts[0],
                    ...
                    case QuicPacketType.INITIAL:  # Initial packet
                        ...
                        header_parts = struct.unpack_from(fmt_string, datagram_data)
                        packet_len = decode_variable_length_int(
                            header_parts[-1]
                        ).to_bytes(packet_len_len, "big")
                        ...
                        decrypted_header = remove_header_protection(
                            header_type=QuicHeaderType.LONG,
                            sample=sample,
                        ...
                        fmt_string += (
                            str(
                                int.from_bytes(packet_len, "big") - decrypted_header[-1]
                            )
                            + "s"
                        )
                        header_parts = struct.unpack_from(fmt_string, datagram_data)
                        ...
                        total_packet_len = (
                            ...
                            + 4
                            + 1
                            + int.from_bytes(dcid_len, "big")
                            + 1
                            + int.from_bytes(scid_len, "big")
                            + token_len_len
                            + token_len
                            + packet_len_len
                            + decrypted_header[-1]
                        ...
                        packet = LongQuicPacket(
                            ...
                            scid=scid,
                            token_len=token_len,
                            token_len_bytes=token_len_bytes,
                            token=token,
                    ...
                    case (
                    ...
                    ):  # Handshake and 0-RTT Packets have the same fields (see RFC 9000)
                        packet_len_len = get_variable_length_int_length(
                            struct.unpack_from(fmt_string + "s", datagram_data)[-1]
                        )
                        fmt_string = fmt_string + str(packet_len_len) + "s"
                        ...
                        if packet_type == QuicPacketType.HANDSHAKE:
                            ...
                            packet = LongQuicPacket(
                                packet_type=QuicPacketType.RTT_O,
                                version=version,
                                dcid_len=dcid_len,
                                dcid=dcid,
                                scid_len=scid_len,
                    ...
                    case QuicPacketType.RETRY:
                        ...
                        packet = LongQuicPacket(
                            ...
                            retry_integ_tag=retry_integ_tag,
                            isserver=isserver,
                            first_byte=datagram_data[0],
                            ts=in_packet.timestamp,
                        )
                        total_packet_len = (
                            1
            ...
            case QuicHeaderType.SHORT:  # if header is short header
                ...
                fmt_string += (
                    str(
                        len(datagram_data)
                        - (1 + len(guessed_dcid) + decrypted_header[-1])
                    )
                    + "s"
                )
                header_parts = struct.unpack_from(fmt_string, datagram_data)
                ...
                total_packet_len = (
                    1 + len(guessed_dcid) + decrypted_header[-1] + len(payload)
                )

                packet = ShortQuicPacket(
                    packet_type=QuicPacketType.RTT_1,
                    dcid=guessed_dcid,
                    isserver=isserver,
                    packet_num=decrypted_header[1],
                    key_phase=key_phase,
