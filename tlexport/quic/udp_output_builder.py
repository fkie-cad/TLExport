...
class UdpOutputBuilder:
    ...
    def __init__(
        self,
        decrypted_records: list[list[bytes, Type[Frame], bool]],
        server_ip,
        client_ip,
        server_port,
        client_port,
        ...
        for record in self.decrypted_records:
            ...
            if isserver:
                packet = (
                    ...
                    / UDP(sport=self.server_port, dport=self.client_port)
                    / Raw(record[0])
                )
            else:
                packet = (
                    Ether(dst=self.server_mac_addr, src=self.client_mac_addr)
