...
class UdpOutputBuilder:
    # TODO: change associated data for frames
    def __init__(self, decrypted_records: list[list[bytes, Type[Frame], bool]], server_ip, client_ip, server_port,
                 client_port, server_mac_addr,
        ...
        for record in self.decrypted_records:
            ...
            if isserver:
                packet = Ether(src=self.server_mac_addr, dst=self.client_mac_addr) / IP(src=self.server_ip,
                    ...
                    sport=self.server_port, dport=self.client_port) / Raw(record[0])
            else:
                packet = Ether(dst=self.server_mac_addr, src=self.client_mac_addr) / IP(dst=self.server_ip,
                                                                                        src=self.client_ip) / UDP(
                    dport=self.server_port, sport=self.client_port) / Raw(record[0])
