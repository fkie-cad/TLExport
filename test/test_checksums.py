...
class TestChecksums(TestCase):
    ...
    def test_ones_complement_checksum(self):
        self.assertEqual(
            ones_complement_checksum(self.string_1), bytearray.fromhex("c20a")
        )
        self.assertEqual(
            ones_complement_checksum(self.string_2), bytearray.fromhex("e197")
        )
        self.assertNotEqual(
            ones_complement_checksum(self.false_string_1), bytearray.fromhex("ab12")
        )

        self.assertEqual(
            ones_complement_checksum(self.zeroes), bytearray.fromhex("ffff")
        )
