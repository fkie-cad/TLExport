...
class TestQuicFrameGeneration(TestCase):
    def setUp(self):
        self.quic_packet = QuicPacket(
            QuicHeaderType.LONG, QuicPacketType.HANDSHAKE, False, first_byte=None, ts=0
        )
    ...
    def test_crypto_frame(self):
        parsed_frames = frames.parse_frames(
            bytes.fromhex(
                "0200414800000600405a020000560303b7be3c8d397996b7264f25fe5f624f4ac37e901a5acd517d408972a6ddb1cda800130100002e00330024001d0020813355ac9760c2dc4e7df93de0d5d9c120f58f38a381706c73d6177705e24a3c002b00020304"
            )
            + bytes.fromhex("0000000000010001"),
            self.quic_packet,
        )
    ...
    def test_stream_frame(self):
        parsed_frames = frames.parse_frames(
            bytes.fromhex(
                "080300041f018001000006800100000740640801c00000044fc25e00c0000000997dd1d8c0000000af79f09b02e68e"
            ),
            self.quic_packet,
        )
    ...
    def test_handshake_done_packet(self):
        parsed_frames = frames.parse_frames(
            bytes.fromhex(
                "1e0740410095028230a8f84f9402b7e252734ad7cc45d4e291c4909931164d2c15d769c0504390b2198dc477ba147f7184c67ab5e27f69667733df75fd6bb8ff8d8ca0273b18010008e01f26872f1ff3d8f60b2d543c097238b39865dd3e7b5880"
            ),
            self.quic_packet,
        )
        ...
        self.assertEqual(
            bytes.fromhex("f60b2d543c097238b39865dd3e7b5880"),
            new_connection_id_frame.stateless_reset_token,
        )

    def test_datagram_frame(self):
        parsed_frames = frames.parse_frames(
            bytes.fromhex("310454657374"), self.quic_packet
        )
