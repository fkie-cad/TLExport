...
class TestVariableLengthInt(TestCase):
    def test_decode_variable_length_int(self):
        ...
        self.assertEqual(
            4611686018427387903,
            decode_variable_length_int(b"\xff\xff\xff\xff\xff\xff\xff\xff"),
        )
        self.assertEqual(
            151288809941952652,
            decode_variable_length_int(bytes.fromhex("c2197c5eff14e88c")),
        )
        self.assertEqual(
            494878333, decode_variable_length_int(bytes.fromhex("9d7f3e7d"))
        )
